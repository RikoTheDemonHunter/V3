...
-- 💾 CONFIGURATION / DATA SAVING SYSTEM
local ConfigFile = "AveryHub_Config.json"
local SavedFlags = {}

if isfile and isfile(ConfigFile) then
	pcall(function()
		local rawData = readfile(ConfigFile)
		SavedFlags = HttpService:JSONDecode(rawData) or {}
	end)
end
...
local function SaveConfig()
	if writefile then
		pcall(function()
			local encoded = HttpService:JSONEncode(SavedFlags)
	...
	function Tabs:Tab(name)
		...
		function Sections:Section(name)
			...
			function Elements:Toggle(name, callback)
				...
				local toggle = SavedFlags[name] or false
				local debounce = false

				if toggle then
					ToggleState.Text = "X"
					task.spawn(pcall, callback, toggle)
				end
